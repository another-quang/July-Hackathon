...
test.describe("ABN lookup", () => {
  test("finds a business for a valid ABN in the sample data", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("displays the matching business details for a valid ABN", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    // Scope assertions to the result card: the always-visible "Browse businesses"
    // list and the generated summary also contain "Active" / "VIC 3000", so an
    // unscoped getByText would match multiple elements.
    const resultCard = page.getByRole("article", { name: /search result/i });
    await expect(
      resultCard.getByRole("heading", {
        name: /example manufacturing pty ltd/i,
      }),
    ).toBeVisible();
    await expect(resultCard.locator(".status")).toHaveText("Active");
    await expect(
      resultCard.locator("dd", { hasText: /vic 3000/i }),
    ).toBeVisible();
  });
  ...
  test("supports keyboard-only navigation from the field to submission", async ({
    page,
  }) => {
    await page.goto("/");
